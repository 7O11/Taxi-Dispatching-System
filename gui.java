...
class node {// 结点信息
	...
	public node(int _NO, int _depth,node pNode) {
...
class guiInfo {
	...
	public synchronized void pointbfs(int root,int dst) {// 单点广度优先搜索
		...
		for (int i = 0; i < 6400; i++)
			view[i] = false;
		queue.add(new node(x, 0, null));
//		Vector<Vector<Point>> a = new Vector<Vector<Point>>();
//		shortpath = new Vector<Point>();
		while (queue.size() > 0) {
			node n = queue.get(0);
			view[n.NO] = true;
			for (int i = 1; i <= 4; i++) {
				...
				if (next >= 0 && next < 6400 && view[next] == false && graph[n.NO][next] == 1) {
					view[next] = true;
					queue.add(new node(next, n.depth + 1, n));// 加入遍历队列
					D[x][next] = n.depth + 1;
					D[next][x] = n.depth + 1;
//					if(next==dst)	return ;
				}
...
//		}
	...
	public synchronized Vector<Point> distance(int x1, int y1, int x2, int y2){
		int src = x1*80+y1;
		int dst = x2*80+y2;
		int[] offset = new int[] { 0, 1, -1, 80, -80 };
		boolean[] view0 = new boolean[6405];
//		Vector<Integer> nodelist = new Vector<Integer>();
		node[] nodes = new node[6400];
		queue0 = new Vector<Integer>();
		nodequeue = new Vector<node>();
		shortpath = new Vector<Point>();
		for(int i=0;i<6400;i++) {
			nodes[i] = null;
		}
		...
		nodequeue.add(endp);
		while(queue0.size()>0) {
			int pathdeatail = queue0.get(0);
			node tempnode = nodequeue.get(0);
			view0[pathdeatail] = true;
			for(int i=1;i<=4;i++) {
				int next = pathdeatail + offset[i];
				if(next >= 0 && next < 6400 && (D[src][pathdeatail]-D[src][next])==1 && graph[pathdeatail][next]==1 && view0[next] == false) {
					queue0.add(next);
					int getflowx0 = pathdeatail/80;
					int getflowy0 = pathdeatail%80;
					int getflowx1 = next/80;
					int getflowy1 = next%80;
//					int flow = guigv.GetFlow(getflowx0, getflowy0, getflowx1, getflowy1);
					int flow = FlowFunc.GetFlow(getflowx0, getflowy0, getflowx1, getflowy1);
//					shortway[pathdeatail][next] = flow;
//					shortway[next][pathdeatail] = flow;
					view0[next] = true;
					if(nodes[next]!=null) {
						node nodei = nodes[next];
						if(nodei.minflow>(tempnode.minflow+flow)) {
							nodei.minflow = tempnode.minflow + flow;
							nodei.pre = tempnode;
						}
					} else {
						nodes[next] = new node(next, 0, tempnode);
						nodes[next].minflow = tempnode.minflow + flow;
//						nodes[next].pre = tempnode;
					}
					nodequeue.add(nodes[next]);
				}
			}
			queue0.remove(0);
			nodequeue.remove(0);
		}
		shortpath.add(new Point(src/80, src%80));
		node tempnode = nodes[src];
		while(tempnode.pre!=null) {
			tempnode = tempnode.pre;
			shortpath.add(new Point(tempnode.NO/80, tempnode.NO%80));
		}
		return shortpath;
	}
...
class guigv {
	...
	public static guiInfo m0 = new guiInfo();//For Special Taxi
	...
	public static void AddFlow(int x1,int y1,int x2,int y2,int flow){//增加一个道路流量
		synchronized (guigv.flowmap) {
			...
			//添加流量
			guigv.flowmap.put(Key(x1,y1,x2,y2), count+flow);
			guigv.flowmap.put(Key(x2,y2,x1,y1), count+flow);
		}
	}
...
class myform extends JFrame {// 我的窗体程序
	...
	public myform() {
		...
		button1.setBounds(10, 770, 100, 40);// 设置按钮的位置
		...
		button2.setBounds(120, 770, 100, 40);// 设置按钮的位置
		...
		button3.setBounds(230, 770, 100, 40);// 设置按钮的位置
		...
		button4.setBounds(340, 770, 100, 40);
		...
		check1.setBounds(450, 770, 80, 40);
		...
		check2.setBounds(530, 770, 80, 40);
		...
		drawboard.setBounds(10, 10, 750, 750);// 设置大小
...
class TaxiGUI {// GUI接口类
	...
	public void MapUpdate(int[][] map, int size) {
		// 复制地图
		for (int i = 0; i < size; i++) {
			for (int j = 0; j < size; j++) {
				guigv.m.map[i][j] = map[i][j];
	...
	public void SetTaxiStatus(int index, Point point, int status) {
		...
		FlowFunc.AddFlow(gt.x, gt.y, point.x, point.y, 1);
		gt.x = point.x;
		gt.y = point.y;
		gt.status = status;
	}
	...
	public Vector<Point> RequestTaxi(int index, Point src, Point dst, int type) {
		if(type==0) {
			return guigv.m.distance(src.x, src.y, dst.x, dst.y);
		} else {
			return guigv.m0.distance(src.x, src.y, dst.x, dst.y);
		}
			...
			if (di == 0) {// 在同一水平线上
				...
				if (status == 0) {// 关闭
					if (guigv.m.map[p.x][p.y] == 3) {
						...
						System.out.println("["+gv.getFormatTime()+"]Warning: Request status already exist!");
					}
				} else if (status == 1) {// 打开
